...
describe('de belofte aan Noor', () => {
  ...
  it.each(['open', 'sheltered'] as const)('past route %s toe op precies het volgende level', (route) => {
    ...
    expect(state.journey).toEqual({ phase: 'travelling', route, memory: null, shelter: null })
  ...
  it.each(['open', 'sheltered'] as const)(
    ...
    (route) => {
      ...
      expect(arrival.adventure.active?.story.fragments.arrival.text).toContain('In het donker')
      const rest = chooseAdventureOption(arrival, 1, createSeededRandom(1), 200_000)
      const approach = chooseAdventureOption(rest, 1)
      const next = chooseAdventureOption(approach, 0, createSeededRandom(1), 300_000)
  ...
  it.each([
    ...
    expect(arrival.adventure.active?.story.fragments.arrival.text).toContain(
      tone === 'persistent' ? 'steentje' : tone === 'fluent' ? 'verkeerd op te vouwen' : 'precies in de vouw',
    )
  ...
  it('bewaart de herinnering, herstelt de gewone regels en begint de klok pas na het gesprek', () => {
    ...
    expect(state.timing.timeReliefMs).toBe(0)
    state = enterAdventure(playLevel(state))
    state = chooseAdventureOption(state, 0)
    state = chooseAdventureOption(state, 0, createSeededRandom(3), 400_000)
    expect(state.journey.phase).toBe('complete')
    expect(state.levelRules).toEqual(getLevelRules())
    const later = startNextLevel({ ...state, status: 'stage-win' }, createSeededRandom(3), 500_000)
  ...
  it('voegt herinneringen toe zonder gedeelde verhaalteksten te veranderen en wist ze bij herstart', () => {
    ...
    expect(createGameState().journey).toEqual({ phase: 'unmet', route: null, memory: null, shelter: null })
  })
})
